...
void build_sensor_event_data(void *context, enum inv_icm20948_sensor sensortype, uint64_t stamp, const void *data, const void *arg)
{
  ...
  case INV_SENSOR_TYPE_GRAVITY:
    ...
    // Gravity vector
    //grav_x = event.data.acc.vect[0];
    //grav_y = event.data.acc.vect[1];
    //grav_z = event.data.acc.vect[2];
...
int TeensyICM20948::init(TeensyICM20948Settings settings)
{
  ...
  dataReadyPin = settings.drdy_pin;
  ...
  //if (settings.fusion.gravity){ seal |= 0b00000010; };
  if (settings.enable_gyroscope)
  {
    seal |= 0b00000100;
  }
  ...
  // if (settings.fusion.eulerAngles!){ seal |= 0b00010000 };
...
void TeensyICM20948::enableDMPInterrupt(int pin, void (*isr)()) //(void isr(void), int mode)
{
  inv_icm20948_set_int1_assertion(&icm_device, 1);
  attachInterrupt(pin, isr, FALLING);
}

void TeensyICM20948::disableDMPInterrupt(int pin) //(void isr(void), int mode)
{
  inv_icm20948_set_int1_assertion(&icm_device, 0);
  detachInterrupt(pin);
}

void TeensyICM20948::sleep()
{
  inv_icm20948_sleep_mems(&icm_device);
}

void TeensyICM20948::wakeup()
{
  inv_icm20948_wakeup_mems(&icm_device);
}

short TeensyICM20948::identifyInterrupt()
{
  short int_read_back;
  inv_icm20948_identify_interrupt(&icm_device, &int_read_back);
  return int_read_back;
}


std::vector<float> TeensyICM20948::readGyroData()
{
  std::vector<float> gyro = {gyro_x, gyro_y, gyro_z};
  gyro_data_ready = false;
  return gyro;
}

std::vector<float> TeensyICM20948::readAccelData()
{
  std::vector<float> accel = {accel_x, accel_y, accel_z};
  accel_data_ready = false;
  return accel;
}

std::vector<float> TeensyICM20948::readMagData()
{
  std::vector<float> mag = {mag_x, mag_y, mag_z};
  mag_data_ready = false;
  return mag;
}

std::vector<float> TeensyICM20948::readQuatData()
{
  std::vector<float> quat = {quat_x, quat_y, quat_z, quat_w};
  quat_data_ready = false;
  return quat;
}

std::vector<float> TeensyICM20948::getDMPData()
{
  std::vector<float> buffer;
  task();

  if (accelDataIsReady())// && initialSettings.enable_accelerometer)
  {
    std::vector<float> accel = readAccelData();
    buffer.insert(buffer.end(), accel.begin(),accel.end());
  }

  if (gyroDataIsReady())// && initialSettings.enable_gyroscope)
  {
    std::vector<float> gyro = readGyroData();
    buffer.insert(buffer.end(), gyro.begin(),gyro.end());
  }

  if (magDataIsReady())// && initialSettings.enable_magnetometer)
  {
    std::vector<float> mag = readMagData();
    buffer.insert(buffer.end(), mag.begin(), mag.end());
  }

  if (quatDataIsReady())// && initialSettings.enable_quaternion)
  {
    std::vector<float> quat = readQuatData();
    buffer.insert(buffer.end(), quat.begin(), quat.end());
  }
  return buffer;
}
